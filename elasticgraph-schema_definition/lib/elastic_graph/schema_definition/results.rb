...
require "elastic_graph/schema_definition/indexing/sourced_update_targets_resolver"
  ...
  module SchemaDefinition
    ...
    class Results < Support::MemoizableData.define(:state)
      ...
      def build_runtime_metadata
        sourced_update_targets_by_type_name = Indexing::SourcedUpdateTargetsResolver.new(schema_def_state: state).resolve

        object_types_by_name = all_types
          .select { |t| t.respond_to?(:graphql_fields_by_name) }
          .to_h { |type| [type.name, (_ = type).runtime_metadata(sourced_update_targets_by_type_name.fetch(type.name) { [] })] }
